      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <span v-if="item.property === 'btn'" class="center">
            <el-button type="primary" size="mini">修改应用</el-button>
            <el-button type="primary" size="mini">导出应用</el-button>
...
export default {
  ...
  data () {
    return {
      ...
      list: [],
      listTitle: [
        { property: 'name', label: '应用名', width: 150 },
  ...
  methods: {
    cellClick (row, column) {
      if (column.label === '应用名') {
        this.$router.push({ path: `/customers/${this.customerId}/dashboards/${row.id.id}`, query: { title: row.name } })
      }
    },
    resetForm (formName) {
